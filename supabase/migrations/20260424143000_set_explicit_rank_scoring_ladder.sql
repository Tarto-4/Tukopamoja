-- Set an explicit rank ladder for multiplayer scoring.
-- 1st=100%, 2nd=90%, 3rd=80%, then taper down to a 20% floor.
    ...
    select
      ...
      case
        ...
        when r.time_taken_ms >= (v_time_limit_sec * 1000) then 0
          ...
          v_max_points * case least(r.answer_rank, 9)
            when 1 then 1.0::numeric
            when 2 then 0.9::numeric
            when 3 then 0.8::numeric
            when 4 then 0.7::numeric
            when 5 then 0.6::numeric
            when 6 then 0.5::numeric
            when 7 then 0.4::numeric
            when 8 then 0.3::numeric
            else 0.2::numeric
          end
        )::int
      end as new_points
